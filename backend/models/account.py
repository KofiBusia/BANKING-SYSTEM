...
class Account(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'ledger_balance': float(self.ledger_balance) if self.ledger_balance is not None else 0.0,
            'interest_rate': float(self.interest_rate) if self.interest_rate is not None else 0.0,
            'minimum_balance': float(self.minimum_balance) if self.minimum_balance is not None else 0.0,
